...
class AudioplayersExamplePage extends StatefulWidget {
  @override
  _AudioplayersExamplePageState createState() =>
      _AudioplayersExamplePageState();
}

class _AudioplayersExamplePageState extends State<AudioplayersExamplePage> {
  ...
  Widget localAsset() {
    return SingleChildScrollView(
      child: _Tab(
        children: [
          const Text("Play Local Asset '国际歌.mp3':"),
          _Btn(
              txt: 'Play',
              onPressed: () =>
                  audioCache.play('images/package/audioplayers/guojige.mp3')),
          const Text("Play Local Asset (via byte source) '国际歌.mp3':"),
          _Btn(
            txt: 'Play',
            onPressed: () async {
              final bytes = await (await audioCache
                      .loadAsFile('images/package/audioplayers/guojige.mp3'))
                  .readAsBytes();
              audioCache.playBytes(bytes);
            },
          ),
          const Text("Loop Local Asset '国际歌.mp3':"),
          _Btn(
              txt: 'Loop',
              onPressed: () =>
                  audioCache.loop('images/package/audioplayers/guojige.mp3')),
          const Text("Loop Local Asset (via byte source) '国际歌.mp3':"),
          _Btn(
            txt: 'Loop',
            onPressed: () async {
              final bytes = await (await audioCache
          ...
          _Btn(
              ...
              onPressed: () =>
                  audioCache.play('images/package/audioplayers/guojige.mp3')),
          const Text("Play Local Asset In Low Latency '国际歌.mp3':"),
          _Btn(
            txt: 'Play',
            onPressed: () {
              audioCache.play('images/package/audioplayers/guojige.mp3',
          ...
          const Text(
            "Play Local Asset Concurrently In Low Latency '国际歌.mp3':",
          ),
          _Btn(
            txt: 'Play',
            onPressed: () async {
              await audioCache.play(
                'images/package/audioplayers/guojige.mp3',
                mode: PlayerMode.LOW_LATENCY,
              );
              await audioCache.play(
                'images/package/audioplayers/guojige.mp3',
  ...
  Future<int> _getDuration() async {
    final uri =
        await audioCache.load('images/package/audioplayers/guojige.mp3');
      ...
      builder: (BuildContext context, AsyncSnapshot<int> snapshot) {
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            ...
            return Text(
              '国际歌.mp3 duration is: ${Duration(milliseconds: snapshot.data!)}',
            );
          default:
            return Container();
        }
  ...
  Widget notification() {
    return _Tab(
      children: [
        const Text("Play notification sound: '国际歌.mp3':"),
        _Btn(
          txt: 'Play',
